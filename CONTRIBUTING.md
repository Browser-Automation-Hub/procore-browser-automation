# Contributing to Procore Browser Automation
...
- **Broken selectors**: Procore updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real Procore instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.PROCORE_URL}/path/to/section`);
...
When you find working selectors for Procore, document them:

```javascript
// Procore selectors (verified YYYY-MM-DD)
...
Procore updates its UI periodically. When selectors break:
